...
<body class="h-screen w-full flex overflow-hidden bg-gray-50">
    ...
    <div class="hidden lg:flex w-1/2 bg-school-green relative flex-col justify-between p-16 text-white overflow-hidden">
        ...
        <div class="z-10 flex items-center gap-3">
            <div
                class="w-10 h-10 bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg flex items-center justify-center text-school-yellow">
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center p-6">
        <div class="w-full max-w-[400px] bg-white p-10 rounded-2xl shadow-xl border border-gray-100">
            ...
            @if(session('success'))
            <div class="mb-4 bg-green-50 border border-green-200 text-green-600 px-4 py-3 rounded relative text-sm"
                role="alert">
            ...
            <form class="space-y-5" method="POST" action="{{ route('create-session') }}">
                @csrf <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5" for="email">Email Address</label>
                    <input type="email" id="email" name="email" value="{{ old('email') }}"
                        class="block w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-school-green focus:border-transparent focus:bg-white transition-all duration-200 @error('email') border-red-500 @enderror"
                        placeholder="admin@sdnegeri1.sch.id" required autofocus>
                    @error('email')
                ...
                <div>
                    ...
                    <input type="password" id="password" name="password"
                        class="block w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-school-green focus:border-transparent focus:bg-white transition-all duration-200 @error('password') border-red-500 @enderror"
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        <input id="remember_me" type="checkbox" name="remember"
                            class="h-4 w-4 text-school-green focus:ring-school-green border-gray-300 rounded">
