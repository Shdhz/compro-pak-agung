...
@section('title', "Berita & Artikel - " . ($school['NAMA_SEKOLAH'] ?? 'Sekolah'))
        ...
        <div class="mb-16">
            <div
                ...
                <div class="relative h-64 lg:h-auto overflow-hidden">
                    ...
                    {{-- Thumbnail Headline --}}
                    <img src="{{ $headline->thumbnail ? asset('storage/'.$headline->thumbnail) : 'https://via.placeholder.com/1000x600?text=No+Image' }}"
                        alt="{{ $headline->judul }}"
                ...
                <div class="p-8 lg:p-12 flex flex-col justify-center">
                    <div class="flex items-center gap-4 text-sm text-gray-500 mb-4">
                        <span class="flex items-center gap-1">
                            ...
                            {{ \Carbon\Carbon::parse($headline->created_at)->translatedFormat('d F Y') }}
                        </span>
                        <span class="w-1 h-1 bg-gray-300 rounded-full"></span>
                        <span class="text-teal-600 font-bold">{{ $headline->category->nama ?? 'Umum' }}</span>
                    </div>
                    <h2
                        class="text-2xl md:text-3xl font-black text-teal-900 mb-4 leading-tight group-hover:text-teal-700 transition">
                        <a href="{{ route('compro.read-news', $headline->slug) }}">{{ $headline->judul }}</a>
                    </h2>
                    <p class="text-gray-600 mb-6 line-clamp-3">
                        {{ $headline->summary }}
                    </p>
                    <a href="{{ route('compro.read-news', $headline->slug) }}"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div class="lg:col-span-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <article
                        ...
                        <div class="relative h-48 overflow-hidden">
                            {{-- Thumbnail Item --}}
                            <img src="{{ $item->thumbnail ? asset('storage/'.$item->thumbnail) : 'https://via.placeholder.com/600x400?text=No+Image' }}"
                                alt="{{ $item->judul }}"
                        ...
                        <div class="p-6 flex flex-col grow">
                            ...
                            <a href="{{ route('compro.read-news', $item->slug) }}"
                ...
                <div class="mt-12 flex justify-center">
                    @if ($news_list->hasPages())
                    <nav class="flex items-center gap-2">

                        {{-- Tombol Previous --}}
                        @if ($news_list->onFirstPage())
                        <span
                            class="w-10 h-10 flex items-center justify-center rounded-lg border border-gray-200 text-gray-300 cursor-not-allowed">
                            &laquo;
                        </span>
                        @else
                        <a href="{{ $news_list->previousPageUrl() }}"
                            class="w-10 h-10 flex items-center justify-center rounded-lg border border-gray-200 text-gray-400 hover:border-teal-600 hover:text-teal-600 transition">
                            &laquo;
                        </a>
                        @endif

                        {{-- Tombol Angka --}}
                        @foreach ($news_list->getUrlRange(1, $news_list->lastPage()) as $page => $url)
                        @if ($page == $news_list->currentPage())
                        <span
                            class="w-10 h-10 flex items-center justify-center rounded-lg bg-teal-600 text-white font-bold shadow-md">
                            {{ $page }}
                        </span>
                        @else
                        {{-- Batasi agar tidak terlalu panjang jika halaman banyak --}}
                        @if ($page == 1 || $page == $news_list->lastPage() || abs($page - $news_list->currentPage()) <
                            2) <a href="{{ $url }}"
                            class="w-10 h-10 flex items-center justify-center rounded-lg border border-gray-200 text-gray-600 hover:border-teal-600 hover:text-teal-600 transition">
                            {{ $page }}
                            </a>
                            @elseif(abs($page - $news_list->currentPage()) == 2)
                            ...
                            {{-- Tombol Next --}}
                            @if ($news_list->hasMorePages())
                            <a href="{{ $news_list->nextPageUrl() }}"
                                class="w-10 h-10 flex items-center justify-center rounded-lg border border-gray-200 text-gray-400 hover:border-teal-600 hover:text-teal-600 transition">
                                &raquo;
                            </a>
                            @else
            ...
            <div class="lg:col-span-1 space-y-8">
                    ...
                    <ul class="space-y-2">
                        ...
                        <li>
                            <a href="{{ route('compro.news', ['category' => $cat->slug]) }}"
                                class="flex justify-between items-center text-gray-600 hover:text-teal-700 hover:bg-teal-50 px-3 py-2 rounded-lg transition {{ request('category') == $cat->slug ? 'bg-teal-50 text-teal-700 font-bold' : '' }}">
                                <span>{{ $cat->nama }}</span>
                    ...
                    <div class="space-y-4">
                        ...
                        <a href="{{ route('compro.read-news', $pop->slug) }}" class="flex gap-3 group">
                            <div class="w-20 h-20 rounded-xl overflow-hidden shrink-0">
                                <img src="{{ $pop->thumbnail ? asset('storage/'.$pop->thumbnail) : 'https://via.placeholder.com/200x200' }}"
                ...
                <div class="bg-teal-900 p-6 rounded-3xl shadow-lg text-white relative overflow-hidden">
                    ...
                    <div class="flex flex-wrap gap-2 relative z-10">
                        ...
                        <a href="#"
                            class="bg-teal-800 hover:bg-yellow-400 hover:text-teal-900 px-3 py-1 rounded-full text-xs font-medium transition">
                            #{{ $tag }}
                        </a>
                        @endforeach
                    </div>
